...
class ReplaceIdsVisitor(c_ast.NodeVisitor):
    def __init__(self, var, array, struct, func, update_ratio):
        # remove duplicates..
        ...
        for type, names in zip([VAR_PREFIX, ARR_PREFIX, STRUCT_PREFIX],
                                [self.var, self.array, self.struct]):
            ...
            for idx, name in zip(idxs, names):
                if random.uniform(0, 1) < update_ratio:
                    self.name_mapping[name] = f'{type}{idx}'
